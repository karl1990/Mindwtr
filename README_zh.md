...
- **多设备最佳方案：** WebDAV 或 Mindwtr Cloud（自托管）。由应用控制同步流程，并按条目合并。
- **文件同步（Syncthing/Dropbox 等）：** 可以用，但**冲突是文件级别**，因为 `data.json` 是单文件。
- **文件同步最佳实践：** 尽量避免两台设备同时编辑；切换设备前等待同步完成。如出现冲突，保留最新的 `data.json` 并删除 `data.json.sync-conflict-*`。
